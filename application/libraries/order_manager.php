...
class Order_manager {
    ...
    public function addOrder($status) {
        ...
        if ($this->checkRedirectSubmitOrder($status) == 2) {
            $dataOrder['status'] = 2;
        }
        if ($this->checkRedirectSubmitOrder($status) == 1) {
            $dataOrder['status'] = 1;
        }
        if ($this->checkRedirectSubmitOrder($status) == -1) {
            $dataOrder['status'] = 4;
    ...
    public function checkRedirectSubmitOrder($status) {
        ...
        if (($this->auth['redirect_submit_order'] == 0 && $this->auth['parent_id'] != 0) && $status != 4) {
            return 1;
        }
        return -1;
    }
    ...
    public function checkView($order_id) {
        $this->CI->load->model('order_model');
        $this->CI->load->model('user_model');
        $order_info = $this->CI->order_model->getOrderById($order_id);
        //check user created order
        if ($order_info['user_id']) {
            $user_info = $this->CI->user_model->getUserById($order_info['user_id']);
            //check user exits
            if ($user_info) {
                if ($user_info['parent_id']) {
                    $list_users = $this->CI->user_model->getCompanyUsers($user_info['parent_id']);
                } else {
                    $list_users = $this->CI->user_model->getCompanyUsers($order_info['user_id']);
                }
                $list_company_users = array();
                //check user on company
                foreach ($list_users as $user) {
                    $list_company_users[] = $user['id'];
                }
                if ($this->auth['id'] && in_array($this->auth['id'], $list_company_users)) {
                    return true;
                }
            } else {
                return false;
    ...
    public function checkEdit($order_id) {
        $this->CI->load->model('order_model');
        $this->CI->load->model('user_model');
        if ($order_id && $this->checkView($order_id)) {
            if (!$this->auth['parent_id']) {
                return true;
            } else {
                $order_info = $this->CI->order_model->getOrderById($order_id);
                if ($order_info['status'] == 1 || $order_info['status'] == 4) {
                    return true;
                } else {
                    return false;
                }
    ...
    public function rejectOrder($orderid) {
        ...
        $this->CI->order_model->update($dataUpdate, $orderid);
    }
